<template>
  <div class="form-group SelectInput" :class="{ 'has-error': !!(errorMessage && meta.touched && !isOpen), success: meta.valid }">
    ...
    <multiselect :id="name" v-model="inputValue" :placeholder="t(`${name}.placeholder`)" track-by="identifier" label="name" :options="optionsList" :multiple="options.multiple" :taggable="true" :group-select="true" @input="handleChange" @blur="handleBlur" @open="toggleOpen" @close="toggleOpen" :searchable="true" :hide-selected="true" ref="multiSelect" :class="{ 'has-error': !!(errorMessage && meta.touched && !isOpen), success: meta.valid }" />
