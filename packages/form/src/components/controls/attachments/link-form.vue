    ...
    <div class="card-body">
      <Form   v-slot="{ values, errors, meta }" @submit="onSubmit" >
        <FormErrorSummary v-bind="{ values, errors, meta }">
          <div class="row">
            <div class="col-12">
              <TextInput @change="onChange" v-if="isImage"   :options="{ type:'url', rules: { format: 'url', required: true } }" :name="`${name}.contentUrl`"  :label="t(`.contentUrl`)" :placeholder="t(`.contentUrl.placeholder`)"  :value="contentUrl" :form-ctx="{ errors, meta, values }" />
