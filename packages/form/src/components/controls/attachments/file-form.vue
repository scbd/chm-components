      ...
      <Form  v-slot="{ values, errors, meta }" @submit="onSubmit"  >
        <FormErrorSummary v-bind="{ values, errors, meta }">
          ...
          <div>
            ...
            <TextInput v-if="inputValue?.contentSize"    :name="`${name}.contentSize`"     :label="' '" :placeholder="' '" :value="inputValue.contentSize"    :options="{ type: 'hidden', rules: { type: 'number', max: uploaded.isImage? 300000 : 1000000000, required: true } }" :form-ctx="{ errors, meta, values }"/>
        ...
        <button class="submit-btn" type="submit">{{t('Add file')}}</button>
        <button class="c-btn" type="button" @click="$emit('close-file-form'); reset();">{{t('Cancel')}}</button>
      </Form>
...
async function fileToBlob (file){ return  new Blob([new Uint8Array(await file.arrayBuffer())], {type: file.type }); }
