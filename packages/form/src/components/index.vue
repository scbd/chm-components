<template>
  <div v-if="opts.id" class="card" ref="chmFormEl" :id="opts.id">
    <div class="card-body">
      <div class="mb-3"  >
        <Icons v-once/>
        <SuccessFeedback/>
        ...
        <Form v-if="!feedbackStore.msgs.length" v-slot="{ values, errors, meta }" @submit="onSubmit"  :initial-values="document" >
          ...
          <div v-for="(row, index) in opts.rows" :key="index"  class="row">
            <div class="col" v-for="({ name, is, value, options }, colIndex) in row.columns" :key="colIndex" >
              <component v-if="!isPlainObject(is)" :is="is" :name="name"  :options="options" :value="value" :label="t(name)" :placeholder="t(`${name}.placeholder`)" :form-ctx="{ errors, meta, values, schema:opts, user }"/>
            </div>
          </div>
...
export default {
  ...
  components: { Captcha, Form, Icons, TextInput, LStringArea, ChmSelect,  Attachments,   LString,  ValidationSummary,SuccessFeedback, CheckBox },
  methods   : { isPlainObject, onSubmit, verifyMethod },
  created,  setup
}
...
function setup(props){
  ...
  return { t, chmFormEl, user, document, identifier,  feedbackStore, opts: options }
}
...
<style>
/* @import url("https://cdn.cbd.int/@scbd-chm/icons@0.0.5/dist/style.css"); */
