...
function readRules ({ props, t, computedLabel }) {
  ...
  if(!isString && !isNumber && !isDate) throw new Error('field type not known: must be text or number')

  let rule = isNumber? number() : isDate? date() : string()
  

  if(rules.format === 'url') rule = rule.url(getMsg('url')).max(256, getMsg('max', 256))
  if(rules.format === 'email') rule = rule.email(getMsg('email')).max(256, getMsg('max', 256))
