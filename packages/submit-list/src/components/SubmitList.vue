...
function mounted() {
  const types    = [ 'national', 'reference', 'SCBD' ];
  const subTypes = [
    'strategiesAndReports', 'accessAndBenefitSharing',
    'biosafetyClearingHouse', 'capacityBuildingRecords',
    'bioBridgeInitiativeRecords', 'events', 'organizations',
    'UNDBRecords', 'virtualLibraryRecords', 'SCBDRecords', 'lifeWeb' ];

  for (const groupName of types) { // eslint-disable-line no-restricted-syntax
    const item = { type: groupName, subType: [] };
    data.forEach((element) => {
      if (element.types.includes(groupName)) {
        const subType = { header: undefined, data: [] };
        for (const subName of subTypes) {  // eslint-disable-line no-restricted-syntax
