...
export default {
  ...
  rows: [
    //Organization Info
    {  columns: [ { name: '$id',                           is: 'TextInput',   options: { type: 'hidden',    rules: { required: true } },  value: 'action'  }  ]},

    {  columns: [ { name: 'actor.name',                     is: 'TextInput',   options: { rules: {required: true, max:200}} }]},
    {  columns: [ 
      { name: 'actor.website',                              is: 'TextInput',   options: { type: 'url', rules: {format: 'url', required: true}}},
    ...
    //Thematic Areas & Progress Tracking (Optional) 
    {  columns: [ { name: 'actionDetails.thematicAreas',    is: 'ChmSelect',   options: { type: 'subjects', multiple: true, rules: { required: false } }  }  ] },
    {  columns: [ { name: 'actionDetails.progressMeasured', is: 'LStringArea', options: { rows: 2, rules: { required: false, max: 1000} }  }  ] },
    {  columns: [
      { name: 'partners.name',                              is: 'TextInput',   options: { rules: {max: 100}}  },
      { name: 'partners.website',                           is: 'TextInput',   options: { type: 'url', rules: { format: 'url'}}}        ]
    },
  ...
  getMessages, sanitizers: { 'actor.location': countrySanitizer }
}
