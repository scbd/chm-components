...
export default {
  ...
  rows: [
    //Organization Info
    {  columns: [ { name: 'actor.name',                 is: 'TextInput',   options: { rules: {required: true, max:200}} }]},
    {  columns: [ { name: 'actor.website',              is: 'TextInput',   options: {rules: {required: true}}}]},
    {  columns: [ { name: 'actor.logo',                 is: 'Attachments', options: { type: 'both', multiple: true} }]},
    {  columns: [ { name: 'actor.location',             is: 'ChmSelect',   options: { type: 'countries',  rules: { required: true } }  }  ] },
    {  columns: [ { name: 'actor.type',                 is: 'ChmSelect',   options: { type: 'orgTypes',  rules: { required: true } }  }  ] },
    ...
    //Commitment Details
    {  columns: [ { name: 'actionDetails.operationalAreas', is: 'ChmSelect',   options: { type: 'geoLocations',  rules: { required: true } }  }  ] },
    {  columns: [ { name: 'actionDetails.actionCategories', is: 'ChmSelect',   options: { type: 'actionCategories',  rules: { required: true } }  }  ] },
    {  columns: [ { name: 'actionDetails.sdgs',             is: 'ChmSelect',   options: { type: 'sdgs',  rules: { required: true } }  }  ] },
    {  columns: [ { name: 'actionDetails.aichiTargets',     is: 'ChmSelect',   options: { type: 'aichis',  rules: { required: true } }  }  ] },
