...
import { sourceMap, sdgsShort, schemas } from './data/index';
...
export const { dataSources } = globalProps.options;

export const getData = async (dataSource, noCache = false) => {
  const localData   = noCache || await getFromLocal(validateDataSource(dataSource));
...
export const lookUp = async (type, keys, single = false) => {
  ...
  const returnData = data.filter(({ identifier }) => (Array.isArray(identifier)
    ? (flatKeys.filter((value) => identifier.includes(value))).length
    : flatKeys.includes(identifier)));
...
export const generateAll = async () => { // eslint-disable-line
  ...
  if (saveCache) { $store.setItem(`all-${locale}`, all); }
...
async function getFromLocal(apiName) {
  ...
  if (!(await isExpired(name))) { return $store.getItem(name); }
  return false;
}
...
function flattenKeys(keys) {
  ...
  if (Array.isArray(keys) && containsStrings(keys)) { return keys; }

  if (Array.isArray(keys) && containsObjects(keys)) { return keys.map((k) => k.identifier); }

  if (Array.isArray(keys) && containsArrays(keys)) { return keys.map((k) => k[0]); }

  if (typeof keys === 'object') { return [ keys.identifier ]; }
...
function sanitize(aDataItem) {
  ...
  return omitBy({
    identifier, name, alternateName, image, url,
  }, isNil);
}
...
function sanitizeSdg(aDataItem) {
  ...
  return {
    identifier, image, url, ...getSdgName(aDataItem),
  };
}
...
function padCode(code) {
  ...
  if (!validTypes.includes(type) || !length) { throw new Error('cached-apis.padCode: cade passed must be string or number'); }
