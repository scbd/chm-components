  ...
  computed: {
    ...
    filteredOptions() {
      ...
      if (this.taggable && normalizedSearch.length && !this.isExistingOption(normalizedSearch)) {
        if (this.tagPosition === 'bottom') { options.push({ isTag: true, label: search }); } else { options.unshift({ isTag: true, label: search }); }
      }
  ...
  methods: {
    ...
    select(option, key) {
      ...
      if (option.isTag) {
        ...
        if (this.multiple) { this.$emit('input', this.internalValue.concat([ option ]), this.id); } else { this.$emit('input', option, this.id); }
    ...
    deactivate() {
      ...
      if (this.isOpen) {
        this.isOpen = false;
        /* istanbul ignore else  */
        if (this.searchable) { this.$refs.search.blur(); } else { this.$el.blur(); }
